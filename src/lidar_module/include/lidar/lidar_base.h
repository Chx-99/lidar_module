...
namespace lidar_base
{
    ...
    public:
        ...
        bool connect(std::string *error_msg = nullptr)
        {
            ...
            if (!sendCommandAndWaitAck(FRAME_TO_SPAN(handshake_frame), "握手", &err))
            {
                if (error_msg)
                    *error_msg = err;
                return false;
            ...
            if (!sendCommandAndWaitAck(FRAME_TO_SPAN(laser_frame), "开启激光", &err))
            {
                if (error_msg)
                    *error_msg = err;
                return false;
            ...
            if (!sendCommandAndWaitAck(FRAME_TO_SPAN(imu_frame), "设置IMU频率", &err))
            {
                if (error_msg)
                    *error_msg = err;
                return false;
        ...
        bool sendCommandFrameAndWaitAck(std::span<const uint8_t> frame, const std::string &step_name, std::string *error_msg = nullptr)
        {
            return sendCommandAndWaitAck(frame, step_name, error_msg);
        }
    ...
    private:
        ...
        // 返回: true=成功, false=失败
        // error_msg: 可选的错误信息输出参数
        bool sendCommandAndWaitAck(std::span<const uint8_t> frame, const std::string &step_name, std::string *error_msg = nullptr)
        {
            // 准备promise用于同步等待ACK
            ...
            if (status == std::future_status::timeout)
            {
                ...
                if (error_msg)
                {
                    *error_msg = "雷达 " + sn_ + " " + step_name + " 超时：未收到ACK";
                }
                return false;
            }

            bool result = ack_future.get();
            if (!result && error_msg)
            {
                *error_msg = "雷达 " + sn_ + " " + step_name + " 失败：retcode非0";
            }
            return result;
        }
