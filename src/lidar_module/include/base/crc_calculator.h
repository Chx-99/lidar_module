...
/*	
	该部分是通过查表法计算crc16与crc32，
	crc16使用的是 MCRF4XX 系列芯片使用的 CRC 校验码，使用1024大表查询，初始值为0x4C49
	crc32使用自定义的256字节小表查询 初始值为0x564f580a 这是非标准初始值
	如果希望优化计算速度、减少计算可以将crc32的小表转换为大表（但由于是自定义表，目前不清楚如何生成）
*/


#define crc16_n4(crc, data, table)                 \
	crc ^= data;                                   \
...
#define crc32_n4(crc, data, table)          \
	crc ^= data;                            \
...
namespace crc_tables
{

	inline constexpr uint16_t crc_table_crc16[1024] = {
	...
	// crc32 table
	inline constexpr uint32_t crc_table_crc32[256] = {
...
namespace crc_calculator
{
	inline uint16_t crc16(const uint8_t *data, uint16_t len)
	{
		...
		{
			crc = (crc >> 8) ^ crc_tables::crc_table_crc16[(crc & 0xff) ^ *data++];
			len--;
		}
		...
		{
			len -= 16;
			crc16_n4(crc, ((uint32_t *)data)[0], crc_tables::crc_table_crc16);
			crc16_n4(crc, ((uint32_t *)data)[1], crc_tables::crc_table_crc16);
			crc16_n4(crc, ((uint32_t *)data)[2], crc_tables::crc_table_crc16);
			crc16_n4(crc, ((uint32_t *)data)[3], crc_tables::crc_table_crc16);
			data += 16;
		}
	...
	inline uint32_t crc32(const uint8_t *data, uint16_t len)
	{

		uint32_t seed = 0x564f580a;
		uint32_t crc = seed ^ 0xffffffff;

		while (((uintptr_t)data & 3) && len)
		{
			crc = (crc >> 8) ^ crc_tables::crc_table_crc32[(crc & 0xff) ^ *data++];
			len--;
		}
		...
		{
			len -= 16;
			crc32_n4(crc, ((uint32_t *)data)[0], crc_tables::crc_table_crc32);
			crc32_n4(crc, ((uint32_t *)data)[1], crc_tables::crc_table_crc32);
			crc32_n4(crc, ((uint32_t *)data)[2], crc_tables::crc_table_crc32);
			crc32_n4(crc, ((uint32_t *)data)[3], crc_tables::crc_table_crc32);
			data += 16;
		}
		while (len--)
		{
...
#define crc16(data, len) crc_calculator::crc16(data, len)
#define crc32(data, len) crc_calculator::crc32(data, len)