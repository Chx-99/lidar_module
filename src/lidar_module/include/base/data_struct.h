...
inline T fromSpan(const std::span<const uint8_t>& span) {
    if (span.size() < FRAME_SIZE) throw std::invalid_argument("Span size is smaller than frame size");
    T frame;
...
inline T fromVector(const std::vector<uint8_t>& vec) {
    if (vec.size() < FRAME_SIZE) throw std::invalid_argument("Vector size is smaller than frame size");
    T frame;
