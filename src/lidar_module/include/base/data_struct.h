...
#define FRAME_TO_VECTOR(data) frame_tools::frameToVector(reinterpret_cast<const uint8_t *>(&data), data.length)
...
namespace frame_tools
{
    ...
    // 将消息帧转化为vector<uint8_t>
    inline std::vector<uint8_t> frameToVector(const uint8_t *data, size_t size)
    {
...
#pragma pack(push, 1) // 确保结构体的字节对齐为1字节
    ...
    struct BoardCast
    {
        ...
        inline std::string getBroadCastCode()
        {
    ...
    struct HandShake
    {
        ...
        inline friend std::ostream &operator<<(std::ostream &os, const HandShake &handshake)
        {
            ...
            oss << "user_ip: " << std::dec << static_cast<int>(handshake.user_ip[0]) << "."
                << static_cast<int>(handshake.user_ip[1]) << "."
                << static_cast<int>(handshake.user_ip[2]) << "."
    ...
    struct SingleEchoRectangularData
    {
        ...
        friend std::ostream &operator<<(std::ostream &os, SingleEchoRectangularData &data)
        {
            ...
            oss << std::dec << "[x: " << data.x / 1000.f << " y: " << data.y / 1000.f << " z: " << data.z / 1000.f
                << " intensity: " << static_cast<float>(data.intensity) << " lable: " << std::bitset<8>(data.lable) << "]" << std::endl;
            os << oss.str();
            return os;
        }
    };
    ...
    */
    struct ImuData
    {
        ...
        {
            std::ostringstream oss;
            oss << "[gyro_x: " << data.gyro_x << " gyro_y: " << data.gyro_y
                << " gyro_z: " << data.gyro_z << " acc_x: " << data.acc_x
                << " acc_y: " << data.acc_y << " acc_z: " << data.acc_z << "]" << std::endl;
            os << oss.str();
            return os;
        }
    };
