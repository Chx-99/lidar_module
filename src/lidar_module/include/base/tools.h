...
inline std::string get_interface_name_from_ip(const std::string& ip_address) {
    ...
    for (auto* adapter = adapter_addresses; adapter != nullptr; adapter = adapter->Next) {
        for (auto* unicast = adapter->FirstUnicastAddress; unicast != nullptr; unicast = unicast->Next) {
            ...
            if (addr->sin_addr.s_addr == inet_addr(ip_address.c_str())) { return adapter->AdapterName; }
        }
    }
    ...
    if (getifaddrs(&ifaddr) == -1) { throw std::runtime_error("Failed to get network interfaces."); }
...
inline std::vector<unsigned short> getRandomAvailablePort(int size) {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        try {
            ...
            if (ports.size() == size) { return ports; }
