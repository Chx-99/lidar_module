...
public:
    ...
    LidarBase(const std::string lidar_ip, const std::string local_ip, const std::string sn)
        : lidar_ip_(std::move(lidar_ip)),
          ...
          ack_recv_buffer_(256),
          pointcloud_data_buffer_(512),
          imu_data_buffer_(512),
          state_(LidarState::DISCONNECTED),
          is_running_(false),
          is_receiving_(false) {
        init();
    }
...
protected:
    ...
    void startReceive() {
        if (is_receiving_.load(std::memory_order_acquire)) { return; }
...
private:
    ...
    /**
     * @brief 将点云数据帧转换为ROS PointCloud2消息
     */
    void appendFrameToRosPointcloud() {}

    void appendFrameToRosIMU() {}
    ...
    void init() {
        ...
        remote_endpoint_ = boost::asio::ip::udp::endpoint(boost::asio::ip::make_address(lidar_ip_), 65000);
    ...
    boost::asio::io_context hc_io_context_, pi_io_context_, work_io_context_;
    ...
    std::deque<sensor_msgs::msg::PointCloud2::SharedPtr> pointcloud_data_buffer_;               // 点云数据缓冲区
    std::deque<sensor_msgs::msg::Imu::SharedPtr> imu_data_buffer_;                              // IMU数据缓冲区
