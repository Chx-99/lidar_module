...
#define MEASURE_TIME(operation, iterations, name)                                                                             \
    ...
    std::cout << #name << " (" << iterations << " iterations) 执行时间: " << duration_##name.count() << " 纳秒" << std::endl; \
...
int main()
{
    ...
    MEASURE_TIME(
        volatile auto result = old_version::TAG_MAP[std::make_tuple(0x01, 0x00, 0x01)];
        , iterations, Old_Version_TAG_MAP_Lookup_HandShake)

    MEASURE_TIME(
        volatile auto result = old_version::TAG_MAP[std::make_tuple(0x01, 0x00, 0x03)];
        , iterations, Old_Version_TAG_MAP_Lookup_HeartBeat)

    MEASURE_TIME(
        volatile auto result = old_version::TAG_MAP[std::make_tuple(0x01, 0x01, 0x08)];
    ...
    MEASURE_TIME(
        old_version::Frame<old_version::heartBeat> heart_o;
        , iterations, Old_Version_heartBeat_Creation)
